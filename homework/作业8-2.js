...
var bindEventEnter = function() {
    ...
    todoContainer.addEventListener('keydown', function(event){
        ...
        if(event.key === 'Enter') {
            ...
            /*把元素在todoList 中更新 todolist 其实就是 class ='todo-cell'的数组
            todolist[i] 也就是 'todo-cell' 的位子下标,
            addEventListener 的事件对象一直都是作用元素(即'button' 'span'等)
            的父元素('todo-cell') */
...
var indexOfElement = function(element) {
    // target 的父元素 class = 'todo-cell'
    var parent = element.parentElement
    // target 父元素的父元素 即 id = 'id-div-container'
